...
public class Point implements Serializable , com.fretron.Utils.Geospacial.compressedPolylineUtils.reducer.Point {
  ...
  /**
   * Utility method to export coordinates for use with GeoJSON. This standard requires longitude
   * first. See http://geojson.org/geojson-spec.html#positions
   */
  ...
  public int hashCode() {
    int hash = 5;
    hash = 37 * hash + (int) (Double.doubleToLongBits(this.lat) ^ (Double.doubleToLongBits(this.lat)
        >>> 32));
    hash = 37 * hash + (int) (Double.doubleToLongBits(this.lng) ^ (Double.doubleToLongBits(this.lng)
        >>> 32));
    return hash;
  }
