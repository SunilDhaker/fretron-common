...
//https://maps.googleapis.com/maps/api/directions/json?origin=Disneyland&destination=Universal+Studios+Hollywood4&key=AIzaSyBs7T9fTwzqLFJpJv35xToAbJKwLC3OOfo
  ...
  private static String getApiUrl(String statrLocation, String endLocation, Boolean isDrivingMode) {
    //driving by default
    return (isDrivingMode) ? API_URL + "origin=" + statrLocation + "&destination=" + endLocation
        + "&key=" + API_KEY :
        API_URL + "origin=" + statrLocation + "&destination=" + endLocation + "&key=" + API_KEY
            + "&mode=walking";
  }


  public static HashMap<String, Object> getDirectionPolyLinePoints(String startLocation,
      String endLocation, Boolean isDrivingMode) throws Exception {
