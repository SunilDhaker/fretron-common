package com.fretron.Utils.Geospacial.GoogleDirectionPolylineUtil;

import com.fretron.Model.PointAtTime;
import com.fretron.Utils.Geospacial.PolylineUtils.Point;
...
public class SnapToRoadUtil {
  ...
  private static String getUrl(String points) {
    ...
    return "https://roads.googleapis.com/v1/snapToRoads?path=" + query + "&interpolate="
        + isInterpolate + "&key=" + api_key;
  }
    ...
    try {
      ...
      if ((snappedPoints.length() > 0)) {
        ...
        for (int index = 0; index < snappedPoints.length(); index++) {
          ...
          JSONObject location = jsonObj.getJSONObject("location");
          list.add(new PointAtTime(timestamp, location.getDouble("latitude"),
        ...
        System.out.println(
            "SnapToRoad snapped points size different from original, may be interpolation is true"
                + (points.size() - snappedPoints.length()));
      }
