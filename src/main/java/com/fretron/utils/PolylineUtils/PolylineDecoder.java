...
public class PolylineDecoder {
	...
	public List<LitePosition> decodeInPositions(String encoded, double precision) {
		...
		while (index < encoded.length()) {
			...
			LitePosition p = new LitePosition((double) lat / precision, (double) lng / precision , 0d,0d , "",0l ,"" , "" , null,null ,false);
			track.add(p);
		}
		return track;
	}
	...
	public List<LitePosition> decodeTimeAwarePolylineInPositions(String encoded, double precision) {
		...
		while (index < encoded.length()) {
			...
			long dtime =  ((lresult & 1) != 0 ? ~(lresult >> 1) : (lresult >> 1));
			time += dtime;
			LitePosition p = new LitePosition((double) lat / precision, (double) lng / precision , 0d,0d , "",time ,"" , "" , null,null ,false);
			track.add(p);
		}
		return track;
	}
