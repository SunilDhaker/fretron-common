        ...
        try {
            ...
            if (polyline != null) {
                ...
                if (points.size() > 0) {
                    ...
                    TimeAwarePolyline compressedPolylineObj = OnlinePolylineEncoder.INSTANCE.extendTimeAwarePolyline(new TimeAwarePolyline("", "", new PointAtTime(0L, 0d, 0d), polylineObj.getIsAssumed(), 1), compressedPointList.get(0).getX(), compressedPointList.get(0).getY(), compressedPointList.get(0).getT());
                    for (int index = 1; index < compressedPointList.size(); index++) {
                        compressedPolylineObj = OnlinePolylineEncoder.INSTANCE.extendTimeAwarePolyline(compressedPolylineObj, compressedPointList.get(index).getX(), compressedPointList.get(index).getY(), compressedPointList.get(index).getT());
                    }
